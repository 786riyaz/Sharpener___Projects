...
async function getAiContext(userId, roomId) {
  const messages = await Message.find({ roomId }).populate("sender", "name email").sort({ createdAt: -1 }).limit(10).lean();
  messages.reverse();
  const context =
    messages
      .map((message) => {
        const who = String(message.sender?._id || message.sender) === String(userId) ? "Me" : message.sender?.name || "Other user";
  ...
  const myMessages = await Message.find({ roomId, sender: userId, text: { $ne: "" } })
    ...
    .lean();
  myMessages.reverse();
  const myRecent = myMessages.map((message) => clampText(message.text, 160)).filter(Boolean);
