...
app.get("/api/personal-chats/recent", authenticateToken, async (req, res) => {
  try {
    ...
    const [activeRooms, archivedRooms] = await Promise.all([
      Message.aggregate([{ $match: { chatType: "personal", roomId: roomPattern } }, { $group: { _id: "$roomId", lastMessageAt: { $max: "$createdAt" } } }]),
      ArchivedChat.aggregate([{ $match: { chatType: "personal", roomId: roomPattern } }, { $group: { _id: "$roomId", lastMessageAt: { $max: "$originalCreatedAt" } } }]),
    ]);
...
app.post("/api/groups", authenticateToken, async (req, res) => {
  try {
    ...
    const normalizedEmails = [req.user.email, ...memberEmails.map(normalizeEmail)].filter(Boolean);
