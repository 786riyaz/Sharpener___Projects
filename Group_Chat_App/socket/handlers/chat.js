  ...
  socket.on("join_room", async (payload = {}, callback) => {
    try {
      ...
      const [activeMessages, archivedMessages] = await Promise.all([
        Message.find({ roomId: validatedRoomId }).populate("sender", "name email").sort({ createdAt: -1 }).limit(200),
        ArchivedChat.find({ roomId: validatedRoomId }).populate("sender", "name email").sort({ originalCreatedAt: -1 }).limit(200),
      ]);
      ...
      const messages = [...activeMessages, ...normalizedArchived].sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt)).slice(-200);
      respond(callback, {
        success: true,
